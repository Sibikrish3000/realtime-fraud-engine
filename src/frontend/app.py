...
with st.sidebar:
    ...
    with st.expander("👤 User Profile", expanded=True):
        ...
        job = st.selectbox(
            "Job Title",
            options=sorted(job_names),
            index=sorted(job_names).index("Engineer, biomedical")
            if "Engineer, biomedical" in job_names
            else 0,
        )
    ...
    with st.expander("💳 Transaction Details", expanded=True):
        ...
        category = st.selectbox(
            "Category",
            options=sorted(category_names),
            index=sorted(category_names).index("grocery_pos")
            if "grocery_pos" in category_names
            else 0,
        )
    ...
    with st.expander("🛠️ Advanced: Feature Overrides", expanded=False):
        ...
        override_trans_count = st.number_input(
            "Count (24h)", min_value=0, value=0, help="Leave 0 to use real-time data"
        )
        override_avg_spend = st.number_input(
            "Avg Spend (24h)", min_value=0.0, value=0.0, help="Leave 0 to use real-time data"
        )
        override_user_avg_all_time = st.number_input(
            "User Avg (All Time)", min_value=0.0, value=0.0, help="Leave 0 to use calculated value"
        )
...
if analyze_btn:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if decision == "BLOCK":
                st.markdown(
                    '<div class="status-banner banner-fraud">❌ FRAUD DETECTED - TRANSACTION BLOCKED</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    '<div class="status-banner banner-safe">✅ TRANSACTION APPROVED</div>',
                    unsafe_allow_html=True,
                )

            if is_shadow and decision == "BLOCK":
                st.markdown(
                    '<div class="status-banner banner-shadow">⚠️ SHADOW MODE: Transaction allowed in simulation.</div>',
                    unsafe_allow_html=True,
                )
        ...
        st.error(
            "🛑 Connection Failed: Could not connect to Inference API. Is the server running at http://127.0.0.1:8000?"
        )
